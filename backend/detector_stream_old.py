"""
FocusAI CPU-TURBO - Version 8.7 (ALL BUGS FIXED)
- Fixed double box issue
- Fixed shoulder-only focus (head now included)
- Faster blur blending
...
class CPUTurboDetector:
    def __init__(self):
        logger.info("🚀 Initializing CPU-TURBO v8.7 (ALL BUGS FIXED)...")
        ...
        self.detection_size = 416  # FIXED: Increased from 320
        ...
        self.detect_every_n = 3  # FIXED: Reduced from 5
        self.skip_counter = 0
        
        self.blur_downscale = 4  # FIXED: Reduced from 6
        self.blur_kernel = 15
        ...
        logger.info("✅ v8.7 Ready - All bugs fixed!")
    ...
    def load_video(self, video_path):
        ...
        self.cap = cv2.VideoCapture(video_path, cv2.CAP_ANY)
    ...
    def _track(self, detections, frame_w, frame_h):
        """Track objects with coordinate validation"""
        ...
        for d in detections:
            ...
            best_id = None
            best_iou = MIN_IOU_THRESHOLD  # FIXED: Using constant
    ...
    def detect(self, frame):
        """Detect objects with validated coordinates"""
        
        frame_h, frame_w = frame.shape[:2]
        
        results = self.yolo.predict(
            frame,
            conf=0.25,  # FIXED: Lowered from 0.30 for better detection
            iou=0.5,
    ...
    def blur_correct(self, frame, detections):
        """FIXED: Blur outside, sharp inside - with head coverage"""
        ...
        # FIXED: Shift center UP to cover head (40% from top instead of 50%)
        obj_cx = (x1 + x2) // 2 - roi_x1
        obj_cy = int(y1 + obj_h * 0.40) - roi_y1  # 40% from top = head area
        
        # FIXED: Larger vertical ellipse to cover full head and feet
        ...
        # FIXED: Faster integer-based blending
    ...
    def annotate(self, frame, detections):
        """FIXED: No double boxes - only one box per object"""
        ...
        for d in detections:
            ...
            # FIXED: Only draw ONE box per object
            if tid == self.focused_track_id:
                # Draw ONLY green focused box
                color = (0, 255, 0)
                ...
                # SKIP drawing orange box - continue to next detection
                continue
            
            # Draw orange box ONLY for non-focused objects
            color = (0, 165, 255)
    ...
    def get_next_frame(self):
        if not self.cap or not self.is_playing:
            ...
            cv2.putText(blank, "FocusAI v8.7", (230, 200),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
            cv2.putText(blank, "All Bugs Fixed!", (200, 230),
